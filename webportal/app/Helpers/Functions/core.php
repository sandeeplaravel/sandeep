...
function isFromApi()
{
	...
	if (
		...
		&& str_contains(\Route::currentRouteAction(), plugin_namespace('apijc'))
	) {
...
{
	if (isEnabledField('phone')) {
		$loginLabel = t('Enter the OTP received by SMS');
	} else {
		$loginLabel = t('OTP received by Email');
	}
	/*
	if (isEnabledField('email') && isEnabledField('phone')) {
		$loginLabel = t('OTP received by SMS or Email');
	} else {
		if (isEnabledField('phone')) {
			$loginLabel = t('OTP received by SMS');
		} else {
			$loginLabel = t('OTP received by Email');
		}
	} 
