...
abstract class AccountBaseController extends FrontController
{
    ...
    public function leftMenuInfo()
    {
        ...
        $this->skills = Skills::where('user_id', auth()->user()->id)->orderByDesc('id');
        view()->share('countSkills', $this->skills->count());

        $this->education = Education::where('user_id', auth()->user()->id)->orderByDesc('id');
        view()->share('countEducation', $this->education->count());


        $this->experience = Experience::where('user_id', auth()->user()->id)->orderByDesc('id');
        view()->share('countExperience', $this->experience->count());
    }
}
