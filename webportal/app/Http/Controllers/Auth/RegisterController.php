...
use App\Http\Requests\PhoneExistRequest;//Added by Vimal
use App\Http\Requests\EmailExistRequest;//Added by Vimal
...
class RegisterController extends FrontController
{
	...
	/**
	 * Check phone number is exist or not in database .
	 *
	 * @return message
	 */
	public function checkPhoneExist(PhoneExistRequest $request)
	{
	 ...
	 * @return message
	 */
	public function checkEmailExist(EmailExistRequest $request)
	{
	...
	public function register(RegisterRequest $request)
	{
		...
		if ($emailVerificationRequired || $phoneVerificationRequired) {
			...
			if ($emailVerificationRequired) {
				...
				$emailResendVerificationMessage = $this->showReSendVerificationEmailLink($user, 'user');
			}
			
			// Sending verifcation SMS for phone
			if ($phoneVerificationRequired) {
			    
			    $SmsApi = new SmsApi();
			
				$response = $SmsApi->sendSMS($user->phone_token, $request->phone);
				
				// Send Verification Code by SMS
				$this->sendVerificationSms($user);
				
				// Show the Re-send link
				$phoneResendVerificationMessage = $this->showReSendVerificationSmsLink($user, 'user');
		...
		// Redirection
		// $data = ['emailResendMessage' => $emailResendVerificationMessage, 'phoneResendMessage' => $phoneResendVerificationMessage]
		// <a href="' . lurl('verify/' . $entityRef['slug'] . '/' . $entity->id . '/resend/sms') . '" class="btn btn-warning">' . t("Re-send") . '</a>';;
		return redirect($nextUrl . "?userId=" . $user->id);
	}
